...
class VG(DETECTION):
    def __init__(self, db_config, split):
        super(VG, self).__init__(db_config)
        ...
        self._vg_dir = os.path.join(data_dir, "vg")

        self._label_dir  = os.path.join(self._vg_dir, "annotations")
        self._label_file = os.path.join(self._label_dir, "vg_{}.json")
        self._label_file = self._label_file.format(self._dataset)

        self._image_file = "{}"#os.path.join(self._vg_dir, "{}")
        ...
        #]
        self._cat_ids = list(range(1, 151))
        self._classes = {
        ...
        self._cache_file = os.path.join(cache_dir, "vg_{}.pkl".format(self._dataset))
        self._load_data()
